...
restore_leases() { api PUT /api/v1/config "$original" >/dev/null 2>&1 && confirm_pending >/dev/null 2>&1 || true; }
...
phase "4.5-cleanup"
require "original lease set restored" api PUT /api/v1/config "$original"
