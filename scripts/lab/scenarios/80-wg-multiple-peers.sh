...
check "live peer traffic intact" retry 60 mr "ping -c1 -W3 10.6.0.10 >/dev/null 2>&1"
check "live peer handshake is recent" check_wg_recent wg0 90
phase "4.5-cleanup"
cfg="$(api GET /api/v1/config)"
new="$(echo "$cfg" | python3 -c '
import json,sys
