...
sleep 2
check "peer added to canonical config" api GET /api/v1/config | grep -q 'lab-rot-peer'
...
sleep 2
check "peer removed from canonical config" sh -c "! api GET /api/v1/config | grep -q lab-rot-peer"
