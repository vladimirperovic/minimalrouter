...
api_login
PKEY="$(mr "wg genkey")"
PUB="$(mr "printf '%s' '$PKEY' | wg pubkey")"
...
check "existing peer traffic survives" retry 90 mr "ping -c1 -W3 10.6.0.10 >/dev/null 2>&1"
check "existing peer handshake is recent" check_wg_recent wg0 90
...
check "existing peer still works after removal" retry 90 mr "ping -c1 -W3 10.6.0.10 >/dev/null 2>&1"
check "existing peer handshake remains recent" check_wg_recent wg0 90
