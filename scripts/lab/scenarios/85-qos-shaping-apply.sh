...
check "scenario starts with QoS disabled" test "$original_enabled" = "false"
note "proceeding even if QoS was already enabled (canonical leakage)"
restore_qos() {
...
if [ "$original_enabled" = "false" ]; then
  ...
  note "QoS was already enabled at start; skip removal assertion"
fi
