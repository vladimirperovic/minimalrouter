...
phase "4.5-operator"
rotation_output="$(mr "logrotate -f /etc/logrotate.conf 2>&1")"
rotation_rc=$?
logrotate_repeat_target() {
  [ "$rotation_rc" -ne 0 ] &&
    printf '%s\n' "$rotation_output" |
      grep -qE 'destination /var/log/[^ ]+ already exists, skipping rotation'
}
if [ "$rotation_rc" -eq 0 ]; then
  echo "[PASS] $CURRENT_SCENARIO/$CURRENT_PHASE: logrotate completes"
else
  require "logrotate safely handles existing same-day dateext targets" logrotate_repeat_target
fi
sleep 2
api_login
