...
code="$(H "echo '$candidate_b64' | base64 -d | curl -sk --max-time 30 -o $body_file -w '%{http_code}' -b $API_COOKIE -X PUT -H 'X-CSRF-Token: $csrf' -H 'Content-Type: application/json' --data-binary @- $MR_API/api/v1/config")"
check "cross-subnet live change rejected with 422" test "$code" = "422"
check "response directs operator to recovery console" H "grep -qi 'recovery console' $body_file"
