...
check "existing peer still carries traffic" retry 60 mr "ping -c1 -W3 10.6.0.10 >/dev/null 2>&1"
# Disabling an unrelated peer does not force the existing peer to renegotiate;
# traffic liveness is the meaningful invariant here.
phase "4.5-cleanup"
cfg="$(api GET /api/v1/config)"
new="$(echo "$cfg" | python3 -c '
import json,sys
