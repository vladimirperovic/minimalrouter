...
for ns in "$router_ns" "$wan_ns" "$lan_ns"; do ip -n "$ns" link set lo up; done
...
ip netns exec "$router_ns" dnsmasq --keep-in-foreground --conf-file=/dev/null --user=root --interface=lan0 --bind-interfaces --listen-address=192.0.2.1 --dhcp-authoritative --dhcp-range=192.0.2.100,192.0.2.150,255.255.255.0,1h --dhcp-option=3,192.0.2.1 --dhcp-option=6,192.0.2.1 --address=/router.test/192.0.2.1 --no-resolv --dhcp-leasefile="$OUTPUT_DIR/dnsmasq.leases" >"$OUTPUT_DIR/dnsmasq.log" 2>&1 &
dnsmasq_pid=$!
sleep 1
kill -0 "$dnsmasq_pid"

ip netns exec "$lan_ns" busybox udhcpc -i lan0 -s "$OUTPUT_DIR/udhcpc-script.sh" -n -q -t 5 -T 1 >"$OUTPUT_DIR/udhcpc.log" 2>&1
lan_address="$(ip -n "$lan_ns" -4 -o address show dev lan0 scope global | awk '{print $4}')"
case "$lan_address" in 192.0.2.*/*) ;; *) echo "DHCP did not assign a LAN address: $lan_address" >&2; exit 1;; esac

dns_answer="$(ip netns exec "$lan_ns" dig +time=2 +tries=1 +short @192.0.2.1 router.test A | tail -n1)"
[ "$dns_answer" = "192.0.2.1" ] || { echo "DNS test returned $dns_answer" >&2; exit 1; }
...
[ -n "$packet_loss" ] && [ "$packet_loss" = "0" ] || { echo "Unexpected packet loss" >&2; exit 1; }
...
sleep 1
kill -0 "$http_pid"
if ip netns exec "$wan_ns" timeout 4 nc -z -w 2 "${lan_address%/*}" 9090; then echo "Unsolicited WAN-to-LAN connection was accepted" >&2; exit 1; fi

ip netns exec "$wan_ns" nmap -Pn -n --max-retries 1 --host-timeout 30s -p 22,53,80,443,8080,8443,51820 -oG "$OUTPUT_DIR/wan-scan.gnmap" 198.18.0.2 >"$OUTPUT_DIR/wan-scan.txt"
if grep -q '/open/' "$OUTPUT_DIR/wan-scan.gnmap"; then echo "Unexpected port exposed on WAN" >&2; cat "$OUTPUT_DIR/wan-scan.gnmap" >&2; exit 1; fi
...
jq -n --arg lan_address "$lan_address" --arg dns_answer "$dns_answer" --arg latency_avg_ms "$latency_avg_ms" --arg packet_loss_percent "$packet_loss" --argjson tcp_bits_per_second "$tcp_bps" --argjson udp_loss_percent "$udp_loss" '{lan_address:$lan_address,dns_answer:$dns_answer,latency_avg_ms:($latency_avg_ms|tonumber),packet_loss_percent:($packet_loss_percent|tonumber),tcp_bits_per_second:$tcp_bits_per_second,udp_loss_percent:$udp_loss_percent,parallel_tcp_streams:64,wan_open_ports:0}' >"$OUTPUT_DIR/summary.json"
