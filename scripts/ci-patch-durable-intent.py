...
text = path.read_text()
old = '''\t\tResponse: apply.ApplyResponse{
\t\t\tID: "tx-valid-record", Success: true, Verified: true,
\t\t},
\t\tCompletedAt: time.Now(),'''
new = '''\t\tResponse: apply.ApplyResponse{
\t\t\tID: "tx-valid-record", Success: true, Verified: true,
\t\t},
\t\tStartedAt: time.Now().Add(-time.Second), CompletedAt: time.Now(),'''
if text.count(old) != 1:
    raise SystemExit("valid record fixture not unique")
text = text.replace(old, new, 1)
old = '''\t\t{name: "missing completion", mutate: func(r *transactionRecord) { r.CompletedAt = time.Time{} }},'''
new = '''\t\t{name: "missing start", mutate: func(r *transactionRecord) { r.StartedAt = time.Time{} }},
\t\t{name: "completion before start", mutate: func(r *transactionRecord) { r.CompletedAt = r.StartedAt.Add(-time.Second) }},'''
if text.count(old) != 1:
    raise SystemExit("missing completion fixture not unique")
text = text.replace(old, new, 1)
text += '''
...
path = Path("cmd/router-applyd/persistence_outcome_test.go")
text = path.read_text()
old = '''\t"testing"'''
new = '''\t"testing"
\t"time"'''
if text.count(old) != 1:
    raise SystemExit("persistence outcome import was not unique")
text = text.replace(old, new, 1)
text += '''
