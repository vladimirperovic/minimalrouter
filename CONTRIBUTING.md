...
is not maintained by a large networking company, and it does not require
contributors to be expert kernel or firewall developers.
...
unattended production firewall unless the release notes explicitly say so.

Please do not present Minimal Router OS as a complete pfSense replacement. The
current goal is a focused, understandable home-router appliance with safe
defaults and a small resource footprint.

## Before changing code

1. Read `README.md`, `ARCHITECTURE.md`, and `SECURITY.md`.
2. Search existing issues and architecture decisions.
...
4. For a change to a trust boundary, persistence model, update strategy,
   privilege model, or configuration lifecycle, propose an ADR first.

A maintainer may suggest a smaller first contribution. This is intended to make
review easier, not to discourage participation.
...
installation smoke test when the required Docker privileges are available:
...
- Never commit real credentials, private keys, tokens, backups, runtime state,
  databases, snapshots, public IP addresses, device names, or private network
  inventories.
...
- documentation or OpenAPI changes when applicable.

Draft pull requests are welcome. It is completely acceptable to ask for help or
say that a change is incomplete.
...
- authorization and secret handling are reviewed;
...
`SECURITY.md` for private reporting instructions.