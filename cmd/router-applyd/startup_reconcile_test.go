...
func TestReconcileStartupQuarantinesCorruptLastGoodAndRecovers(t *testing.T) {
	corruptErr := errors.New("invalid character 'g' looking for beginning of value")
	quarantined := false
	firstRun := false
	err := reconcileStartup(startupReconcileHooks{
		loadLastGood: func() (*config.SystemConfig, error) { return nil, corruptErr },
		...
		pendingExists: func() (bool, error) { return false, nil },
		restoreRuntime: func(config.SystemConfig) error {
			t.Fatal("confirmed runtime restore must not run for a corrupt last-good")
			return nil
		},
		...
		clearPending: func() error {
			t.Fatal("pending clear must not run without a pending marker")
			return nil
		},
	})
	if err != nil {
		t.Fatalf("corrupt last-good must recover via first-run path, got error: %v", err)
	}
	if !quarantined {
		t.Fatal("corrupt last-good was not quarantined")
	}
	if !firstRun {
		t.Fatal("first-run recovery runtime was not restored")
	}
}

func TestReconcileStartupFailsClosedWhenQuarantineUnavailable(t *testing.T) {
	corruptErr := errors.New("corrupt last-good")
	quarantineErr := errors.New("simulated rename failure")
	err := reconcileStartup(startupReconcileHooks{
		loadLastGood: func() (*config.SystemConfig, error) { return nil, corruptErr },
		quarantineLastGood: func() error {
		...
		pendingExists: func() (bool, error) { return false, nil },
		restoreRuntime: func(config.SystemConfig) error {
			t.Fatal("runtime restore must not run when quarantine fails")
			return nil
		},
		clearPending: func() error { return nil },
	})
	if !errors.Is(err, quarantineErr) {
		t.Fatalf("error = %v, want wrapped quarantine error", err)
	}
}

func TestReconcileStartupQuarantineDiscardsOrphanedPending(t *testing.T) {
	corruptErr := errors.New("corrupt last-good")
	quarantined := false
	cleared := false
	err := reconcileStartup(startupReconcileHooks{
		...
		pendingExists: func() (bool, error) { return true, nil },
		restoreRuntime: func(config.SystemConfig) error {
			t.Fatal("confirmed runtime restore must not run")
			return nil
		},
		restoreFirstRun: func() error { return nil },
		clearPending: func() error {
			cleared = true
			return nil
		},
	...
	if !quarantined || !cleared {
		t.Fatalf("quarantined=%v cleared=%v, want both true", quarantined, cleared)
	}
}
