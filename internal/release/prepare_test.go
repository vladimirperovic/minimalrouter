...
func TestExtractReleaseArchivePreservesReviewedModes(t *testing.T) {
	if runtime.GOOS == "windows" {
		// The appliance is Linux; Windows does not carry POSIX permission bits,
		// so this assertion can only be made where it means something.
		t.Skip("POSIX file modes are not represented on this platform")
	}
