...
test("opens DNS Filter and the visual Kids weekly scheduler", async ({ page, isMobile }) => {
  ...
  await expect(page.getByLabel("Mon 19:00 allowed")).toHaveAttribute("aria-pressed", "true");
  await expect(page.getByLabel("Mon 18:00 blocked")).toHaveAttribute("aria-pressed", "false");
  await expect(page.getByLabel("Sat 03:00 allowed")).toHaveAttribute("aria-pressed", "true");
  ...
  await page.getByLabel("Mon 18:00 blocked").click();
  await expect(page.getByLabel("Mon 18:00 allowed")).toHaveAttribute("aria-pressed", "true");
});
